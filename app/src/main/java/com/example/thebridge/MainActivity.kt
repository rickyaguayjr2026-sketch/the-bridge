...
fun BridgeApp() {
    ...
    when (screen) {
        AppScreen.COVENANT -> CovenantIntro(onCovenantAccepted = { screen = AppScreen.MODE_SELECT })
        AppScreen.MODE_SELECT -> ModeSelector(onModeSelected = { mode -> userMode = mode; screen = AppScreen.HOME })
        AppScreen.HOME -> HomeScreen(userMode = userMode!!)
    }
}
